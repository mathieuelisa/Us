...
| 38 | **Fond dérivé du thème incohérent entre écrans** : seul le hub utilise le pastel du thème personnel (`THEME_PASTEL_BACKGROUND`, demande explicite) ; Suivi santé, Démarches, Ensemble et Mon partenaire affichent toujours le fond fixe par rôle (`ROLE_BACKGROUND`). À harmoniser si l'un des deux mécanismes doit devenir la règle générale. | [`src/app/(tabs)/index.tsx`](../src/app/%28tabs%29/index.tsx), [`src/features/hub/constants.ts`](../src/features/hub/constants.ts) |
