...
| 40 | **`PlainInput` maison, généralisée à tout le formulaire de l'app** : `Input`/`TextField` de Hero UI Native n'ont jamais reçu les variables `--field-background`/`--field-border`/etc. pour nos 4 thèmes custom (sauge/corail/lavande/ocre) — seuls `light`/`dark` les ont via le stylesheet de la lib. Résultat : champ sans fond visible dès qu'un de nos thèmes est actif. Remplacé partout par un `TextInput` brut (fond blanc, bordure `border-accent`). Même sort pour `Avatar` (`profil.tsx`), remplacé par un cercle `bg-accent` maison. À reconsidérer si HeroUI expose un jour officiellement ces variables pour des thèmes personnalisés. | [`src/components/plain-input.tsx`](../src/components/plain-input.tsx) |
