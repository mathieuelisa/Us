...
| 39 | **Avatar du profil sans fond coloré visible** : `Avatar size="lg" color="accent"` (Hero UI Native) n'affiche qu'un « ? » sans cercle de fond perceptible sur `src/app/(tabs)/profil.tsx` — pas noir (bug résolu), mais pas non plus le rendu attendu. À investiguer si un vrai avatar/photo doit un jour s'afficher là. | [`src/app/(tabs)/profil.tsx`](../src/app/%28tabs%29/profil.tsx) |
